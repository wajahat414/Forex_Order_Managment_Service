...
namespace OrderManagmentService
{
    struct DataWriterContainer
    {
        ...
        distributed_ats_utils::topic_tuple_ptr<OrderResponseReport> _order_response_report_tuple; // Fixed typo and type
        ...
        DataWriterContainer(distributed_ats_utils::basic_domain_participant_ptr participant_ptr)
        {
            ...
            _order_response_report_tuple = participant_ptr->make_topic<OrderResponseReportPubSubType, OrderResponseReport>(ORDER_RESPONSE_TOPIC_NAME);

            _order_response_report_dw = participant_ptr->make_data_writer(_order_response_report_tuple);
        }
    };
