...
bool Application::onExecutionReportRecieved(const DistributedATS_ExecutionReport::ExecutionReport &executionReport)
{
    auto response_report = Adapter::executionReportToOrderReponseReport(executionReport);

    auto ret = _dataWriterContainer->_order_response_report_dw->write(&response_report);

    if (ret != eprosima::fastdds::dds::RETCODE_OK)
    {

        std::cout << "Unable to to publish new Order to DDS Order Response report" << std::endl;
        return false;
    }
    return true;
}
