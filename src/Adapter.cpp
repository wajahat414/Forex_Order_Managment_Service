...
Adapter::requestToNewOrderSingle(const OrderRequest &order)

{
  ...
  if (order.quantity() <= 0.0) {
    std::cerr << "❌ Invalid quantity for order: " << order.order_id()
              << std::endl;

    throw std::invalid_argument("Invalid quantity in order: " +
                                order.order_id());
  }

  new_order.OrderQty(order.quantity());
  if (order.type() != OrderType::MARKET && order.price() <= 0.0) {
    std::cerr << "❌ Invalid price for non-market order: " << order.order_id()
              << std::endl;
    throw std::invalid_argument("Invalid price for non-market order: " +
                                order.order_id());
  }
  ...
  // ✅ Enhanced timestamp handling - use original order timestamp or current
  // time
  uint64_t transaction_time =
      (order.timestamp() > 0) ? order.timestamp() : getCurrentUTCTimestamp();
  ...
  std::cout << "✅ Converted OrderRequest to NewOrderSingle: "
            << order.order_id() << " for " << order.symbol() << std::endl;

  return new_order;
}

OrderResponseReport Adapter::executionReportToOrderReponseReport(
...
uint64_t Adapter::getCurrentUTCTimestamp() {
  ...
  auto microseconds =
      std::chrono::duration_cast<std::chrono::microseconds>(epoch_time);
