...
bool RiskValidator::initialize(const std::string &user_config_file, const std::string &symbol_config_file)
{
    ...
    std::cout << "RiskValidator initialized with " << user_configs_.size()
              << " users and " << symbol_configs_.size() << " symbols" << std::endl;
    return true;
}

bool RiskValidator::validate_order(const OrderRequest &order, std::string &rejection_reason)
{
...
bool RiskValidator::validate_order_parameters(const OrderRequest &order, std::string &reason)
{
    ...
    if ((order.type() == OrderType::STOP || order.type() == OrderType::STOP_LIMIT) && order.stop_price() <= 0)
    {
...
bool RiskValidator::validate_user_limits(const OrderRequest &order, const UserConfig &user_config, std::string &reason)
{
    ...
    double order_value = order.quantity() * (order.type() == OrderType::MARKET ? 1.0 : order.price());
...
bool RiskValidator::validate_symbol_limits(const OrderRequest &order, const SymbolConfig &symbol_config, std::string &reason)
{
...
bool RiskValidator::load_symbol_configs(const std::string &config_file)
{
    try
    {
        ...
        if (!file.is_open())
        {
            std::cerr << "Cannot open symbol config file: " << config_file << std::endl;
            return false;
        }
...
void RiskValidator::update_user_config(const std::string &user_id, const UserConfig &config)
{
    user_configs_[user_id] = config;
}

void RiskValidator::update_symbol_config(const std::string &symbol, const SymbolConfig &config)
{
