...
OrderListener::OrderListener(std::shared_ptr<RiskValidator> risk_validator,
    ...
    : participant_(nullptr), topic_tuple_(nullptr), reader_tuple_(nullptr),
      risk_validator_(risk_validator), order_router_(order_router) {}

OrderListener::~OrderListener() { cleanup(); }
...
bool OrderListener::init() {
  try {
    ...
    topic_tuple_ =
        participant_
            ->make_topic<OrderRequestPubSubType, OrderRequestPubSubType>(
    ...
    std::cout << "✅ Created topic: " << NEW_ORDER_REQUEST_TOPIC_NAME
              << std::endl;

    // Create data tuple reader with this listener

    reader_tuple_ =
        participant_->make_data_reader_tuple<OrderRequestPubSubType>(
            topic_tuple_, this);
    ...
    std::cerr << "Exception during OrderListener initialization: " << e.what()
              << std::endl;
    LOG4CXX_ERROR(
        logger, "Exception during OrderListener initialization: " << e.what());
    cleanup();
    return false;
  }
}
...
void OrderListener::on_subscription_matched(
  ...
  if (info.current_count_change == 1) {
    ...
    std::cout << "Current count: " << info.current_count
              << ", Total count: " << info.total_count << std::endl;
    LOG4CXX_INFO(logger,
                 "Current count: " << info.current_count
...
void OrderListener::process_order(const OrderRequest &order) {
  ...
  if (!risk_validator_->validate_order(order, rejection_reason)) {
    std::cout << "Order " << order.order_id()
              << " rejected: " << rejection_reason << std::endl;
    LOG4CXX_WARN(logger, "Order " << order.order_id()
  ...
  if (order_router_->route_to_matching_engine(order)) {
    std::cout << "Order " << order.order_id() << " routed to matching engine"
              << std::endl;
    LOG4CXX_INFO(logger,
                 "Order " << order.order_id() << " routed to matching engine");
    ...
    std::cerr << "Failed to route order " << order.order_id()
              << " to matching engine" << std::endl;

    LOG4CXX_ERROR(logger, "Failed to route order " << order.order_id()
                                                   << " to matching engine");
